...
# new label list
LABEL_LIST = [i for i in range(1001, 3000)]
...
def process_instance(instance_mask, image, dual_vae_model, ll_threshold,  overlap_thresh, global_mask, save_path, gt_mask,filter_size=200):
    ...
    if np.sum(instance_patch) < 3000:
        region_bboxes = [bbox]
        print(f"instance_patch sum: {np.sum(instance_patch)}")
    else:
    ...
    for reg_bbox in region_bboxes:
        ...
        if patch_mask.sum() < filter_size and len(region_bboxes) > 1:
        ...
        # if ll < -500 and len(region_bboxes) > 1:
        #     continue
        ...
        if np.sum(seg_patch) < filter_size and len(region_bboxes) > 1:
            continue
        ...
        if np.sum(global_patch) == 0:
            ...
            print(f"Merging patch at center {center} with label {merge_label}")
        else:
...
def main_pipeline(image_path, mask_path, gt_path, csv_path, output_dir, dual_vae_model_path, ll_threshold):
    """
    ...
    error_ids = get_error_ids(csv_path)
    ...
    save_path = f"{output_dir}/{base_name}.tif"
    for label in error_ids:
        ...
        final_mask = process_instance(
            ...
            ll_threshold, 0.1,final_mask,save_path,gt_mask
            )
    # Save segmentation patch after processing
    tifffile.imwrite(save_path, final_mask.astype(np.uint16))
...
if __name__ == '__main__':
    ...
    for image_file in tqdm(os.listdir(args.image_path)):
        if image_file.endswith(".tif"):
            ...
            csv_path = os.path.join(args.csv_path, image_file.replace(".tif", ".csv"))
            main_pipeline(image_path, mask_path, gt_path, csv_path,args.output_dir, args.dual_vae_model_path, args.ll_threshold)
